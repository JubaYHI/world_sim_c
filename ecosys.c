...
void bouger_animaux(Animal *la)
{
  ...
  // ajustement des coordonnées après le déplacement (monde torique )
...
void rafraichir_predateurs(Animal **liste_predateur, Animal **liste_proie)
{
  ...
  while (tmp)
  {
    ...
    if (Proie_trouvee) // si un prédateur a  trouvéé une proie
    {
      ...
      Proie_trouvee->energie = -1000; //évite que la proie continue a vivre en mangeant de l'herbe si elle a été mangée par un prédateur 
     }
    (tmp)->energie -= 1;
    if ((tmp)->energie < 0)
    {
